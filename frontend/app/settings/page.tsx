      ...
      <main className="w-full max-w-6xl mx-auto px-6 pt-10 pb-12">
        <div className="mb-8 text-center">
          <div className="inline-flex items-center gap-2 mb-3">
            <Settings className="w-7 h-7 text-(--hf-accent)" />
            <h1 className="text-2xl font-bold text-(--hf-text)">设置</h1>
          </div>
        ...
        {loading ? (
          <div className="h-56 rounded-2xl border border-(--hf-border) bg-(--hf-surface) flex items-center justify-center text-sm text-(--hf-text-dim)">
          ...
          <div className="space-y-5">
            {groups.map(group => (
              <section key={group.title} className="rounded-2xl border border-(--hf-border) bg-(--hf-surface) p-5 shadow-(--lp-shadow-raised)">
                <div className="mb-4">
                  <h2 className="text-sm font-semibold text-(--hf-text)">{group.title}</h2>
                  <p className="mt-1 text-xs text-(--hf-text-muted)">{group.description}</p>
                </div>
                  ...
                  {group.fields.map(field => {
                    ...
                    return (
                      <label key={field.path} className="flex flex-col gap-1.5 min-w-0">
                        <span className="text-xs font-medium text-(--hf-text-muted)">{field.label}</span>
                        {field.type === 'boolean' ? (
                          <select
                            value={String(Boolean(value))}
                            onChange={event => updateField(field, event.target.value === 'true')}
                            className="h-10 rounded-lg border border-(--hf-border) bg-(--hf-surface) px-3 text-sm text-(--hf-text) outline-none focus:border-(--hf-border-strong)"
                          >
                          ...
                          <input
                            ...
                            placeholder="输入新密钥覆盖"
                            className="h-10 rounded-lg border border-(--hf-border) bg-(--hf-surface) px-3 font-mono text-sm text-(--hf-text) outline-none focus:border-(--hf-border-strong)"
                          />
                        ) : (
                          <input
            ...
            <div className="sticky bottom-4 flex items-center gap-3 rounded-2xl border border-(--hf-border) bg-(--hf-surface)/95 p-3 shadow-lg backdrop-blur">
              {message && (
                <span className="flex items-center gap-1.5 text-sm text-(--hf-accent)">
                  <CheckCircle className="w-4 h-4" />
                  {message}
                </span>
              )}
              {error && (
                <span className="flex items-center gap-1.5 text-sm text-(--hf-error)">
              ...
              <button
                onClick={save}
                disabled={saving}
                className="ml-auto flex items-center gap-2 rounded-xl bg-(--hf-accent) px-5 py-2 text-sm font-medium text-(--hf-accent-ink) shadow-(--lp-shadow-raised) transition-colors hover:opacity-90 disabled:cursor-not-allowed disabled:bg-(--hf-surface-ghost)"
              >
