...
class ImageClient:
    ...
    def generate_image(self,
        ...
        if is_seedream:
            ...
            # --- OpenAI 兼容逻辑（DTMaaS: wan2.7-image / qwen-image-* 等）---
            try:
                logger.info("ImageClient routed to OpenAI-compatible: model=%s", model)
                gpt_size = size.replace('*', 'x') if size else "1024x1024"
                path = self.gpt_client.generate_image(
                    prompt=prompt,
                    size=gpt_size,
                ...
                logger.exception("OpenAI-compatible generation failed: %s", e)
