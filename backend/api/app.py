...
async def lifespan(app: FastAPI):
    logger.info("Starting 造梦 API")
    logger.info("Code directory mounted at /code: %s", settings.CODE_DIR)
    yield
    logger.info("造梦 API shutdown complete")


app = FastAPI(title="造梦", version="2.0.0", lifespan=lifespan)
...
@app.get("/")
async def root():
    return {"service": "造梦", "version": "2.0.0", "health": "/api/health"}
