...
class WorkflowEngine:
    ...
    def stop_session(self, session_id: str):
        ...
        with self._state_lock:
            # 立即清理 active 标记，允许用户停止后重新执行（旧任务会因 stop event 在下个检查点中断）
    ...
    async def execute_stage(self,
        ...
        if not background_item_regeneration:
            with self._state_lock:
                ...
                # 清除旧的停止信号，让新任务能正常启动（session_stop 引用同一 event，clear 后 is_set() 为 False）
                self._stop_events[state.session_id].clear()
